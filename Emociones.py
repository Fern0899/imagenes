...
classes = ['enfado','disgusto','miedo','felicidad','neutral','tristeza','sorpresa']
...
emotionModel = load_model("modelFEC10.h5")
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		(Xi, Yi, Xf, Yf) = box
		(enfado,disgusto,miedo,felicidad,neutral,tristeza,sorpresa) = pred


		label = ''
		# Se agrega la probabilidad en el label de la imagen
		label = "{}: {:.0f}%".format(classes[np.argmax(pred)], max(enfado,disgusto,miedo,felicidad,neutral,tristeza,sorpresa) * 100)
